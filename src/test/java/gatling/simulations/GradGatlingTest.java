...
public class GradGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all grads").get("/api/grads").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new grad")
                        .post("/api/grads")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"gradNaziv\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_grad_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created grad").get("${new_grad_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created grad").delete("${new_grad_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Grad entity").exec(scn);
