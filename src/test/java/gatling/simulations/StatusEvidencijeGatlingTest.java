...
public class StatusEvidencijeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all statusEvidencijes").get("/api/status-evidencijes").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new statusEvidencije")
                        .post("/api/status-evidencijes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"statusEvidencijeNaziv\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_statusEvidencije_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created statusEvidencije").get("${new_statusEvidencije_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created statusEvidencije").delete("${new_statusEvidencije_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StatusEvidencije entity").exec(scn);
