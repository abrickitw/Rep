...
public class NaseljeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all naseljes").get("/api/naseljes").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new naselje")
                        .post("/api/naseljes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"naseljeNaziv\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_naselje_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created naselje").get("${new_naselje_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created naselje").delete("${new_naselje_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Naselje entity").exec(scn);
