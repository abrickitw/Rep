...
public class VrstaUslugeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all vrstaUsluges").get("/api/vrsta-usluges").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new vrstaUsluge")
                        .post("/api/vrsta-usluges")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"vrstaUslugeNaziv\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_vrstaUsluge_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created vrstaUsluge").get("${new_vrstaUsluge_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created vrstaUsluge").delete("${new_vrstaUsluge_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the VrstaUsluge entity").exec(scn);
