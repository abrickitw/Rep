...
public class RasporedGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all rasporeds").get("/api/rasporeds").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new raspored")
                        .post("/api/rasporeds")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"datumUsluge\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_raspored_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created raspored").get("${new_raspored_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created raspored").delete("${new_raspored_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Raspored entity").exec(scn);
