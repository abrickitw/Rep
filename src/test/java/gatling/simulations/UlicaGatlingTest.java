...
public class UlicaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all ulicas").get("/api/ulicas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ulica")
                        .post("/api/ulicas")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"ulicaNaziv\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ulica_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ulica").get("${new_ulica_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created ulica").delete("${new_ulica_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Ulica entity").exec(scn);
